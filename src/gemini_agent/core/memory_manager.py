...
class MemoryManager:
    ...
    def _save_semantic(self):
        with self._lock:
            self.state_manager._save_json(
                self.semantic_file, {"main": self.semantic_memory}
            )
            self._refresh_retrieval_index()
    ...
    def _save_procedural(self):
        with self._lock:
            self.state_manager._save_json(
                self.procedural_file, {"main": self.procedural_memory}
            )
    ...
    def get_relevant_context(
        self, query: str, context: Optional[str] = None, limit: int = 5
    ) -> str:
        ...
        with self._lock:
            ...
            if "text" in event:
                self.vector_store.add_documents(
                    ...
                    metadatas=[
                        {"session_id": session_id, "timestamp": event.get("timestamp")}
                    ],
    ...
    def update_tool_expertise(self, tool_call: ToolCall, execution_time: float):
        with self._lock:
            ...
            if name not in self.procedural_memory.tool_expertise:
                self.procedural_memory.tool_expertise[name] = ToolExpertise(
                    tool_name=name
                )
            ...
            exp.avg_execution_time = (
                (exp.avg_execution_time * (total_calls - 1)) + execution_time
            ) / total_calls
