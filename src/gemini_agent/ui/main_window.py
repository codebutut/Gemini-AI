...
from gemini_agent.config.app_config import AppConfig, ModelRegistry, Role, Theme
...
class GeminiBrowser(QMainWindow):
    ...
    def _setup_sidebar(self) -> None:
        ...
        self.project_explorer.folder_attached.connect(
            lambda p: self.add_attachment(p, "project")
        )
    ...
    def _setup_terminal(self) -> None:
        ...
        self.terminal_dock.visibilityChanged.connect(
            self.on_terminal_visibility_changed
        )
    ...
    def _setup_input_area(self, layout: QVBoxLayout) -> None:
        ...
        keywords = ["/clear", "/help", "/reset", "/conductor", "/search", "/mas"]
    ...
    def setup_settings_menu(self) -> None:
        ...
        # 1. Settings Dialog
        self.settings_menu.addAction("⚙️ Application Settings").triggered.connect(
            self.open_settings
        )
        self.settings_menu.addSeparator()

        # 2. Conductor Orchestrator
        self.settings_menu.addAction("🚀 Conductor Orchestrator").triggered.connect(
            self.open_conductor
        )
        ...
        # 4. Plugins
        self.settings_menu.addAction("🔌 Manage Plugins").triggered.connect(
            self.open_plugins
        )
        ...
        history_menu.addAction("Restore History").triggered.connect(
            self.restore_history
        )

        self.settings_menu.addSeparator()
        self.settings_menu.addAction("🧹 Clear Vector Cache").triggered.connect(
    ...
    def _populate_conductor_menu(self, menu: QMenu) -> None:
        ...
        for cmd in commands:
            ...
            action.triggered.connect(
                lambda checked, c=cmd: self.run_conductor_command(c)
            )
    ...
    def refresh_index(self) -> None:
        ...
        self.index_thread = threading.Thread(target=_bg_index, daemon=True)
    ...
    def run_conductor_command(self, command_name: str) -> None:
        ...
        self.session_manager.update_session_title(
            self.session_manager.current_session_id, f"Conductor: {command_name}"
        )
        self.update_sidebar()
    ...
    def apply_theme(self) -> None:
        ...
        if theme == Theme.DARK.value:
            self.header.btn_toggle_sidebar.setStyleSheet(
                btn_style % ("#555", "#888", "#333")
            )
    ...
    def _get_session_id_from_item(self, item: QListWidgetItem) -> str | None:
        """Extracts the session ID from a list item's user data."""
    ...
    def load_session_from_list(self, item: QListWidgetItem | None) -> None:
        ...
        if item:
            session_id = self._get_session_id_from_item(item)
        else:
        ...
        if session:
            ...
            if len(messages) > 50:
                ...
                info_lbl.setStyleSheet(
                    "color: #888; font-style: italic; margin-left: 50px;"
                )
    ...
    def show_context_menu(self, pos: Qt.AlignmentFlag) -> None:
        ...
        if not item:
            return
        data = item.data(Qt.ItemDataRole.UserRole)
        if not data or (isinstance(data, dict) and data.get("type") != "chat"):
            return

        menu = QMenu()
        menu.addAction("Rename").triggered.connect(lambda: self.rename_session(item))
        menu.addAction("Export to Markdown").triggered.connect(
            lambda: self.export_session_to_markdown(item)
        )
        menu.addSeparator()
    ...
    def delete_session(self, item: QListWidgetItem) -> None:
        """Deletes a session."""
        sess_id = self._get_session_id_from_item(item)
        if sess_id and self.session_manager.delete_session(sess_id):
            self.update_sidebar()
    ...
    def rename_session(self, item: QListWidgetItem) -> None:
        ...
        current_name = session.title if session else item.text().strip()
        new_name, ok = QInputDialog.getText(
            self, "Rename", "Chat Title:", text=current_name
        )
        if ok and new_name:
    ...
    def export_session_to_markdown(self, item: QListWidgetItem) -> None:
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Export Session", default_name, "Markdown Files (*.md)"
        )

        if file_path:
            if Exporter.export_to_file(session, Path(file_path)):
                QMessageBox.information(
                    self, "Export Successful", f"Session exported to {file_path}"
                )
            else:
    ...
    def backup_history(self) -> None:
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self, "Backup History", "conductor_backup.zip", "ZIP Files (*.zip)"
        )
        if file_path:
            if Exporter.create_backup(
                self.session_manager.get_all_sessions(), Path(file_path)
            ):
                QMessageBox.information(
                    self, "Backup Successful", f"History backed up to {file_path}"
                )
            else:
    ...
    def restore_history(self) -> None:
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Restore History", "", "ZIP Files (*.zip)"
        )
        if file_path:
            ...
            if sessions:
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    self.create_new_session()
                    QMessageBox.information(
                        self, "Restore Successful", "History restored successfully."
                    )
            else:
                QMessageBox.critical(
                    self,
                    "Restore Failed",
                    "Failed to restore backup or backup is empty.",
                )
    ...
    async def _perform_semantic_search(self, query: str):
        ...
        self.messages_layout.addWidget(
            MessageBubble(response, is_user=False, theme_mode=self.app_config.theme)
        )
        self.scroll_to_bottom()
    ...
    def _start_worker(
        ...
        self.messages_layout.addWidget(
            MessageBubble(display_text, is_user=True, theme_mode=self.app_config.theme)
        )
        self.scroll_to_bottom()
    ...
    def show_tool_confirmation(
        self, tool_name: str, args: dict, confirmation_id: str
    ) -> None:
        """Shows a confirmation dialog for dangerous tool execution."""
        dialog = DeepReviewDialog(
            tool_name, args, parent=self, theme_mode=self.app_config.theme
        )
        result = dialog.exec()
    ...
    def on_response_success(self, text: str) -> None:
        ...
        self.messages_layout.addWidget(
            MessageBubble(text, is_user=False, theme_mode=self.app_config.theme)
        )
        self.scroll_to_bottom()
    ...
    def on_response_error(self, err: str) -> None:
        ...
        self.messages_layout.addWidget(
            MessageBubble(
                f"**Error:** {err}", is_user=False, theme_mode=self.app_config.theme
            )
        )
        self.scroll_to_bottom()
