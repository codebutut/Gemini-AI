...
class StateManager:
    ...
    def __init__(self, state_dir: Path, cache_manager: Optional[CacheManager] = None):
        ...
        self.agent_states: Dict[str, AgentState] = self._load_json(self.agent_states_file, AgentState)
        self.tool_calls: List[ToolCall] = self._load_json_list(self.tool_calls_file, ToolCall)
        self.user_profiles: Dict[str, UserProfile] = self._load_json(self.user_profiles_file, UserProfile)
        ...
        try:
            with self._lock:
                with file_path.open("w", encoding="utf-8") as f:
                    json.dump(data, f, indent=4, ensure_ascii=False, cls=EnhancedJSONEncoder)
        except Exception as e:
            logger.error(f"Failed to save {file_path}: {e}")

    def save_agent_state(self, session_id: str, agent_name: str, history: List[Dict[str, Any]]):
        key = f"{session_id}_{agent_name}"
        with self._lock:
            state = AgentState(agent_name=agent_name, session_id=session_id, history=history)
            self.agent_states[key] = state
            self._save_json(self.agent_states_file, self.agent_states)
            if self.cache_manager:
                self.cache_manager.set_state(f"agent_state_{key}", state.model_dump(exclude_none=True))

    def load_agent_state(self, session_id: str, agent_name: str) -> Optional[AgentState]:
        key = f"{session_id}_{agent_name}"
        if self.cache_manager:
    ...
    def get_user_profile(self, user_id: str = "default_user") -> UserProfile:
        with self._lock:
            ...
            if self.cache_manager:
                self.cache_manager.set_state(f"user_profile_{user_id}", profile.model_dump(exclude_none=True))
            return profile
    ...
    def get_execution_context(self, task_id: str) -> ExecutionContext:
        with self._lock:
            ...
            if self.cache_manager:
                self.cache_manager.set_state(f"exec_context_{task_id}", context.model_dump(exclude_none=True))
            return context
    ...
    def save_plan(self, session_id: str, plan: Plan):
        with self._lock:
            ...
            if self.cache_manager:
                self.cache_manager.set_state(f"plan_{session_id}", plan.model_dump(exclude_none=True))
