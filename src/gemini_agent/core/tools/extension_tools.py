...
class ManageExtensionArgs(BaseModel):
    ...
    extension_type: str = Field(..., description="Type of extension: 'plugin' or 'mcp'.")
    name: str = Field(..., description="Name of the extension or package.")
    key: str | None = Field(None, description="Configuration key (for 'configure' operation).")
    value: Any | None = Field(None, description="Configuration value (for 'configure' operation).")
    ...
    env: dict[str, str] | None = Field(None, description="Environment variables for MCP server.")
