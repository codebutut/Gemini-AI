    ...
    try:
        ...
        for node in matching_nodes:
            ...
            for _, target, data in G.out_edges(node, data=True):
                connections.append(f"-> {data.get('relation', 'related to')} -> {target}")
            for source, _, data in G.in_edges(node, data=True):
                connections.append(f"<- {data.get('relation', 'related to')} <- {source}")
...
def create_note(title: str, content: str, tags: List[str] = None) -> str:
    ...
    note_data = {
        ...
        "content": content,
        "tags": tags or [],
        "created_at": str(Path(note_path).stat().st_ctime) if note_path.exists() else None # Placeholder
    }
...
def search_notes(query: str, tags: List[str] = None) -> str:
    ...
    for note_file in NOTES_DIR.glob("*.json"):
        try:
            ...
            match = False
            if query and (query.lower() in note['title'].lower() or query.lower() in note['content'].lower()):
                match = True
            if tags:
                if any(tag.lower() in [t.lower() for t in note.get('tags', [])] for tag in tags):
                    match = True
            
            if match:
                results.append(f"Title: {note['title']}\nTags: {', '.join(note.get('tags', []))}\nContent: {note['content'][:200]}...")
        except Exception as e:
